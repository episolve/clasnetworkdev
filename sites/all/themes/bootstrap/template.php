    ...
    'bootstrap_append_element' => array(
      ...
      'user_register_form' => array(
          ...
          'preprocess functions' => array(
              'bootstrap_preprocess_user_register'
          )
      ),
      'user_profile_form' => array(
          ...
          'render element' => 'form',
          'preprocess functions' => array(
              'bootstrap_preprocess_user_profile_form'
          )
      )
  );
}
...
function bootstrap_preprocess_user_profile_form(&$variables) {
    $account = user_load($variables['form']['#user']->uid);
...
function bootstrap_preprocess_html(&$variables) {
    ...
    if (!empty($url_prefix)) {
	    if ($url_prefix == 'dashboard') {
	        $variables['classes_array'][] = 'dashboard';
	    } else if ($url_prefix == 'collection') {
	        $variables['classes_array'][] = 'collection';
	        $variables['classes_array'][] = 'dashboard';
	    } else if ($url_prefix == 'user') {
	        $variables['classes_array'][] = 'public-profile-page';
	    } else if ($url_prefix == 'node') {
	        $variables['classes_array'][] = 'node-view-page';
	        $node = node_load(arg(1));
	        
	        if (!empty($node) && $node->type == NODE_TYPE_CLAS_COLLECTION) {
				$variables['classes_array'][] = 'page-collection';
	        }
		}
    }
}
